...
int main() {
...

    {
        ...
        auto customDerivationPath = TWStringCreateWithUTF8Bytes("m/49'/0'/0'/0/0");
        auto customDerivationPath2 = TWStringCreateWithUTF8Bytes("m/49'/0'/0'/0/1");
        TWPrivateKey* secretPrivateKeyCustom = TWHDWalletGetKey(walletImp, customDerivationPath);

        auto extendedPrivateKey = TWHDWalletGetExtendedPrivateKey(walletImp, TWPurposeBIP49, coinType, TWHDVersionYPRV);
        auto extendedPublicKey = TWHDWalletGetExtendedPublicKey(walletImp, TWPurposeBIP49, coinType, TWHDVersionYPUB);
        cout << "Extended Secret key:  " << TWStringUTF8Bytes(extendedPrivateKey) << endl;
        cout << "Extended Public key:  " << TWStringUTF8Bytes(extendedPublicKey) << endl;
        auto stdDeriviationPath = TWCoinTypeDerivationPath(coinType);

        auto publicKey = TWHDWalletGetPublicKeyFromExtended(extendedPublicKey, customDerivationPath);
        auto publicKey2 = TWHDWalletGetPublicKeyFromExtended(extendedPublicKey, customDerivationPath2);
        TWStringDelete(customDerivationPath);

        cout << "Public key:  " << TWStringUTF8Bytes(TWStringCreateWithHexData(TWPublicKeyData(publicKey))) << endl;
        publicKey = TWPublicKeyCreateWithData(TWPublicKeyData(publicKey), TWPublicKeyTypeSECP256k1);

        auto publicKeyData = TWPublicKeyData(publicKey);
        auto publicKeyData2 = TWPublicKeyData(publicKey2);

        TWDataAppendData(publicKeyData, publicKeyData2);

        auto publicKeyString = TWStringUTF8Bytes(TWStringCreateWithHexData(publicKeyData));
        cout << "Public key joined:  " <<  publicKeyString << " length " << strlen(publicKeyString) << endl;
        //string addressCustom = TWStringUTF8Bytes(TWCoinTypeDeriveAddress(coinType, secretPrivateKeyCustom));
        cout << "Public key assembled: " << TWStringUTF8Bytes(TWStringCreateWithHexData(TWPublicKeyData(TWPublicKeyCreateWithData(publicKeyData, TWPublicKeyTypeSECP256k1)))) << endl;

        string addressCustom = TWStringUTF8Bytes(TWCoinTypeDeriveAddressFromPublicKey(coinType, TWPublicKeyCreateWithData(publicKeyData, TWPublicKeyTypeSECP256k1)));
